...
def create_sum_order_items_df(df):
    sum_order_items_df = df.groupby("product_category_name_english").product_id.sum().sort_values(ascending=False).reset_index()
    return sum_order_items_df

#Create_sum_product()
def create_sum_product_df(df):
    sum_product_df = df.groupby("product_category_name_english").product_id.nunique().reset_index()
...
with st.sidebar:
    ...
    st.image("https://github.com/Raafikurnia/proyek-dicoding-analisis-data-dengan-python/raw/main/keranjang.png")
...
sns.barplot(
        ...
        palette=["#72BCD4" if city == most_common_city else "#D3D3D3" for city in bycity_df['customer_city']]
    )
